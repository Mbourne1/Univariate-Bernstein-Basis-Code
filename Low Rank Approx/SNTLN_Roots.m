...
DTQ_alpha = BuildDTQ(fw_wrt_alpha,alpha_gw_wrt_alpha,t);

% Calculate the derivative of D_{k}TQ_{k} with respect to theta
DTQ_theta = BuildDTQ(fw_wrt_theta,alpha(ite).*gw_wrt_theta,t);
...
DP  = BuildDP_Roots(m,n,alpha(ite),th(ite),opt_col,t,ratio);
...
DY  =   BuildDY_Roots(m,n,t,opt_col,x_ls,alpha(ite),th(ite),ratio);
...
while condition(ite) >(SETTINGS.MAX_ERROR_SNTLN) &&  ite < SETTINGS.MAX_ITERATIONS_SNTLN
    ...
    Partial_DTQ_wrt_alpha = BuildDTQ(fw_wrt_alpha, Partial_gw_wrt_alpha,t);
                
    % Calculate the partial derivative of DTQ with respect to theta.
    Partial_DTQ_wrt_theta = BuildDTQ(fw_wrt_theta, alpha(ite).*gw_wrt_theta,t);
    ...
    DNQ_wrt_alpha = BuildDTQ(Partial_zfw_wrt_alpha, Partial_zgw_wrt_alpha,t);
       
    % Calculate the derivatives of DNQ with respect to theta
    DNQ_wrt_theta = BuildDTQ(Partial_zfw_wrt_theta, alpha(ite).*Partial_zgw_wrt_theta,t);
    ...
    DTNQ_alpha = BuildDTQ(fw_wrt_alpha + Partial_zfw_wrt_alpha,...
        Partial_gw_wrt_alpha + Partial_zgw_wrt_alpha,t);
